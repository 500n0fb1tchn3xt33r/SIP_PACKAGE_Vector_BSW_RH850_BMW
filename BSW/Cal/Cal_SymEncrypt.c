...
# error "Vendor specific version numbers of Cal_SymEncrypt.c and Cal.h are inconsistent"
#endif
...
typedef struct
{
  Cal_AlignType Buffer[CAL_SYMENCRYPT_CONTEXT_BUFFER_SIZE - 1]; /* Generator added one element for the status */
  Cal_AlignType Status;
} Cal_SymEncryptCtxBufStruct;
...
# if ( CAL_NUMBER_OF_SYM_ENCRYPT_CONFIGS != 0 )
STATIC FUNC(P2CONST(Cal_SymEncryptConfigType, AUTOMATIC, CAL_APPL_DATA), CAL_CODE) Cal_SymEncryptGetCfg(Cal_ConfigIdType cfgId);
# endif /* ( CAL_NUMBER_OF_SYM_ENCRYPT_CONFIGS != 0 ) */
...
# if ( CAL_NUMBER_OF_SYM_ENCRYPT_CONFIGS != 0 )
/**********************************************************************************************************************
  Cal_SymEncryptGetCfg
**********************************************************************************************************************/
STATIC FUNC(P2CONST(Cal_SymEncryptConfigType, AUTOMATIC, CAL_APPL_DATA), CAL_CODE) Cal_SymEncryptGetCfg(Cal_ConfigIdType cfgId)
{
  P2CONST(Cal_SymEncryptConfigType, AUTOMATIC, CAL_APPL_DATA) calRtn;
  uint8 i;

  calRtn = NULL_PTR;

  for(i = 0; i < CAL_NUMBER_OF_SYM_ENCRYPT_CONFIGS; i++)
  {
    if(cfgId == Cal_SymEncryptConfig[i].ConfigId)
    {
      calRtn = &Cal_SymEncryptConfig[i];
      break;
    }
  }
 ...
 *  Cal_SymEncryptStart
 **********************************************************************************************************************/
/*! \brief                         This function shall be used to initialize the symmetrical encrypt service of the CAL
 *                                 module.
 ...
 *  \description                   This function shall be used to initialize the symmetrical encrypt service of the CAL
 *                                 module.
...
# if ( CAL_NUMBER_OF_SYM_ENCRYPT_CONFIGS != 0 )
  P2CONST(Cal_SymEncryptConfigType, AUTOMATIC, CAL_APPL_DATA) symEncryptCfg;
  P2VAR(Cal_SymEncryptCtxBufStruct, AUTOMATIC, CAL_APPL_DATA) context;

  /* PRQA S 0310 1 */ /* MD_CAL_11.4 */
  context = ((P2VAR(Cal_SymEncryptCtxBufStruct, AUTOMATIC, CAL_APPL_DATA))contextBuffer); /* Casted Context Buffer */
  symEncryptCfg = Cal_SymEncryptGetCfg(cfgId); /* Pointer to Specific Service Configuration */

  if((symEncryptCfg != NULL_PTR) &&
     (keyPtr        != NULL_PTR) &&
     ( (InitVectorPtr != NULL_PTR) || (InitVectorLength == 0u) ) )
  {
    /* Call the configured Primitive Function */
    calRtn = symEncryptCfg->PrimitiveStartFct(symEncryptCfg->PrimitiveConfigPtr,
                                              context->Buffer,
                                              keyPtr,
                                              InitVectorPtr,
                                              InitVectorLength);
...
/***********************************************************************************************************************
 *  Cal_SymEncryptUpdate
 **********************************************************************************************************************/
/*! \brief                         This function shall be used to feed the symmetrical encryption service with the input
 *                                 data.
 *  \param[in]  cfgId              Holds the identifier of the CAL module configuration which has to be
 ...
 *  \description                   This function shall be used to feed the symmetrical encryption service with the
...
FUNC(Cal_ReturnType, CAL_CODE) Cal_SymEncryptUpdate(Cal_ConfigIdType cfgId,
                                                    Cal_SymEncryptCtxBufType contextBuffer, /* PRQA S 3673 */ /* MD_CAL_16.7 */
...
# if ( CAL_NUMBER_OF_SYM_ENCRYPT_CONFIGS != 0 )
     ...
     (cipherTextLengthPtr != NULL_PTR))
  {
    ...
    {
      /* Call the configured Primitive Function */
      calRtn = symEncryptCfg->PrimitiveUpdateFct(symEncryptCfg->PrimitiveConfigPtr,
                                                 context->Buffer,
                                                 plainTextPtr,
                                                 plainTextLength,
                                                 cipherTextPtr,
...
/***********************************************************************************************************************
 *  Cal_SymEncryptFinish
 **********************************************************************************************************************/
/*! \brief                         This function shall be used to finish the symmetrical encryption service.
 *  \param[in]  cfgId              Holds the identifier of the CAL module configuration which has to be
 *                                 used during the symmetrical encryption computation.
 *  \param[in/out]  contextBuffer  Holds the pointer to the buffer in which the context of this service can be stored.
 *  \param[out]     cipherTextPtr  Holds a pointer to the memory location which will hold the encrypted text.
 ...
 *                                      provided by cipherTextPtr. On returning from this function the amount of data
 *                                      that has been encrypted shall be stored.
 *  \return     CAL_E_OK           - success, encryption data has been accepted.
 ...
 *  \description                   This function shall be used to finish the symmetrical encryption service. If the
...
# if ( CAL_NUMBER_OF_SYM_ENCRYPT_CONFIGS != 0 )
     ...
     (cipherTextLengthPtr != NULL_PTR))
  {
    if(context->Status == CAL_ACT_ACTIVE)                                           /* PRQA S 0310 */ /* MD_CAL_11.4 */
    {
      calRtn = symEncryptCfg->PrimitiveFinishFct(symEncryptCfg->PrimitiveConfigPtr,
                                                 context->Buffer,
                                                 cipherTextPtr,
                                                 cipherTextLengthPtr);
    }
